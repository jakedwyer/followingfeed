...
handle_error() {
    ...
    cd /root/followfeed/docker/api && docker compose down --remove-orphans || true
    exit $exit_code
}
...
cd docker/api && docker compose down --remove-orphans || log "Warning: Cleanup of old containers failed"
