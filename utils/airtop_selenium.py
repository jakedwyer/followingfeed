...
class AirtopRemoteConnection(RemoteConnection):
...
def init_airtop_driver(
    ...
    for attempt in range(max_retries):
        try:
            ...
            custom_executor = AirtopRemoteConnection(
                airtop_api_key=api_key, remote_server_addr=session.data.chromedriver_url
            )

            # Create Remote WebDriver using the URL string
            driver = webdriver.Remote(
                command_executor=session.data.chromedriver_url,
                options=chrome_options,
                keep_alive=True,
            )

            # Replace the default connection with our custom one
            driver.command_executor = custom_executor
